...
function groupCollide(asprite1, spriteGroup) {
	...
	for (var key in spriteGroup)
		...
		var collided = spriteCollide(asprite1, spriteGroup[key]);
		if(collided) {
			collidelist.push(spriteGroup[key]);
		}
	}
	return collidelist;
}
...
function Sprite(img, x, y, width, height, wid) {
	this.x = x;
	this.y = y;
	this.wid = wid;
	this.dx = 0;
...
function GrabbableSprite(img, x, y, width, height, wid) {
	this.x = x;
	this.y = y;
	this.wid = wid;
	...
	// -------------------------------
	this.addSound = function(id, sound) {
	...
	this.notify = function(event) {
		...
		if (event.type == "mousedown") {
			if (collidePoint(this, event.offsetX, event.offsetY)) {
				...
				this.isGrabbed = true;
				currentScene.grabbed = this;
				currentScene.grabbing = true;
			}
		}
		if (event.type == "mouseup" && this.isGrabbed) {
			//if (this.wid < 4) {groupCollide(this, 
			//}
			this.isGrabbed = false;
			currentScene.grabbed = false;
			currentScene.grabbing = false;
			if (this.hasMoved == false) {
				this.playSound(this.wid);
			}
			this.hasMoved = false;

			// if hasCollided:
			//		victim.playSound(this.wid)
		}
		if (event.type == "mousemove") {
			if (this.isGrabbed) {
				newx = event.offsetX;
				...
				for (key in currentScene.layer1) {
					var collision = false;
					if (currentScene.layer1[key] != this) {
						collision = spriteCollide(this, currentScene.layer1[key]);
					}
					if (collision) {
						currentScene.layer1[key].playSound(this.wid);
					}
				}
				//TODO: reset all the objects
		...
		this.playSound = function(id) {
			EvalSound(this.sounds[id]);
		}
		
			
		// TODO: EVALSOUND. deal with in a minute
...
var sprite8 = new GrabbableSprite("img/At Image.png", canvas.width * .60, canvas.height * .60, 63, 42, 7);
sprite8.addSound(0, "audio-AT");
sprite8.addSound(1, "audioBAT");
sprite8.addSound(2, "audioCAT");
scene1.addSprite(sprite8, 1);
//var sprite4 = new GrabbableSprite("img/NT image.png", canvas.width * .60, canvas.height * .60, 100, 63, 4);
//sprite4.addSound(0, "audioANT");
//sprite4.addSound(4, "audio-NT");
//scene1.addSprite(sprite4, 1);
/*
scene1.addSprite(sprite1, 0);
scene1.addSprite(sprite2, 0);
scene1.addSprite(sprite3, 0);
